...
int main(int argc, char *argv[])
{
   ...
   {
      std::cout << "/dev path to i2c bus must be passed" << std::endl;
      exit(-1);
   }
   ...
   // Create a i2c handler
   I2CHandler daemon(argv[1]);

   // Initialize the device
   retval = daemon.init();
   if (retval != 0)
   {
      std::cout << "Error initializing I2C device " << retval << std::endl;
      exit(-2);
   }
